...
For the rest of this chapter, we're going to work on building a map that will help us gain insight into geographic patterns in foreclosure notices by zip code in Maryland. What geographic patterns can we identify?

First, we'll go out and get housing data for each zip code using tidycensus. The variable for total population is B25003_002.  
...
```{r}
md_zip_housing <- get_acs(geography = "zcta",
...
The next step will be to join our housing data to our foreclosure notice data on the zip code column.
...
```{r}
foreclosures_with_housing <- md_zip_housing %>%
  left_join(foreclosure_zip_march_2023, by=c('GEOID'='zip'))

foreclosures_with_housing
```
Our final step before visualization, let's calculate the number of foreclosure notices per 10 owner-occupied housing units and sort from highest to lowest to see what trends we can identify just from the table.
...
foreclosures_with_housing <- md_zip_housing %>%
  left_join(foreclosure_zip_march_2023, by=c('GEOID'='zip')) %>% 
  mutate(pct = march_2023/estimate*10) %>%
  arrange(desc(pct))

foreclosures_with_housing
```

Let's take a look at the result of this table. There is one zip code where there have been 10 foreclosure notices for every 100 owner-occupied units, 21641.
