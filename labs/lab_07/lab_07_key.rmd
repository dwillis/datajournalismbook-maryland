...
**Q3** We're interested in knowing more about Montgomery County zip codes where overdose calls are coming from and the demographic characteristics they possess. In particular, we want to calculate a per capita rate of calls per . To do this, you'll need to:
...
2. Find the ACS variable pertaining to total population (there are several choices here; you want the simplest one).
...
md_zcta_population <- get_acs(geography="zcta", variables = "B01001_001", state='MD', year=2019)
